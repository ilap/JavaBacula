 ...
 * @author ilap
 */
public class VolumeManager extends BaculaObjectManager {
        
    private static VolumeManager instance = null;    
    
    private VolumeManager(int state) {
        super(state);
    }
    
    private static int initState() {
        return BaculaConstants.REFRESHABLE |
               BaculaConstants.DELETEABLE;
    }

    public static VolumeManager getInstance() {
        if (instance == null) {
            instance = new VolumeManager(initState()); 
        } 
        return instance;
    }
    
    public BaculaObject createBaculaItem() {
        return createBaculaItem(BaculaConstants.VOLUME_TYPE);
    }    
}
